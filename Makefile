...
export LH_DEFAULT_SCOPE_MODE ?= LHScopeModePerAppInstall
...
build: generate
	$(MAKE) -C src/packaging/theos THEOS="$(THEOS)" DEBUG=$(THEOS_DEBUG) FINALPACKAGE=$(THEOS_FINALPACKAGE) PACKAGE_VERSION="$(PACKAGE_VERSION)" LH_EMBED_BUILD_SEED=$(LH_EMBED_BUILD_SEED) LH_PREFERENCES_EMBED_BUILD_SEED=$(LH_PREFERENCES_EMBED_BUILD_SEED) LH_DEFAULT_SCOPE_MODE=$(LH_DEFAULT_SCOPE_MODE)

package: package-verify

package-build: generate
	$(MAKE) -C src/packaging/theos THEOS="$(THEOS)" DEBUG=0 FINALPACKAGE=1 PACKAGE_VERSION="$(PACKAGE_VERSION)" LH_STATE_PROVIDER_KIND=LHStateProviderKindPackage LH_EMBED_BUILD_SEED=0 LH_PREFERENCES_EMBED_BUILD_SEED=1 LH_DEFAULT_SCOPE_MODE=$(LH_DEFAULT_SCOPE_MODE) package
