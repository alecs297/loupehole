...
bool LHMitigation_media_library_music_permissioned_empty_install(LHHookBackend *backend, LHPolicyEngine *policy) {
    (void)policy;
    ...
    if (!installed) {
        return LHHookBackendRegisterNoOp(backend, LHModuleID_media_library_music_permissioned_empty);
    }
    return true;
}
