...
bool LHMitigation_photos_library_permissioned_empty_install(LHHookBackend *backend, LHPolicyEngine *policy) {
    (void)policy;
    ...
    if (!installed) {
        return LHHookBackendRegisterNoOp(backend, LHModuleID_photos_library_permissioned_empty);
    }
    return true;
}
