...
bool LHMitigation_personal_data_eventkit_permissioned_empty_install(LHHookBackend *backend, LHPolicyEngine *policy) {
    (void)policy;

    Class storeClass = NSClassFromString(@"EKEventStore");
    if (storeClass == Nil) {
        return LHHookBackendRegisterNoOp(backend, LHModuleID_personal_data_eventkit_permissioned_empty);
    }
