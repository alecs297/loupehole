...
flowchart LR
    ...
    Modules --> Backend
    Modules --> Engine
    Modules --> Tweak["Tweak code + tweakkit helpers"]
    Tweak --> State
    Tweak --> Seeds["Generated policy seeds"]
```
...
| `src/tweakkit/LHTweakValues.c` | Tweak-facing value derivation helpers. |
| `src/tweakkit/LHValueQuantizer.c` | Shared value-shaping utilities. |
...
| `src/tweaks/` | Mitigation installers, hook adapters, and tweak-owned shared helpers. |
...
A tweak can ask the engine to load or create a state blob. State is separated from the hook because hook execution can be frequent, reentrant, or failure-prone. A state key contains generated bytes and a schema version; changing either is a compatibility decision that needs a migration/reset story.
