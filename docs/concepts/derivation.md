...
| `LH_POLICY_SEED(identifier)` | Mitigation source | Mitigation-owned value streams and state keys. |

The generator hashes the build seed plus each policy seed identifier at compile time. Runtime code uses generated bytes, not a policy-key string.
...
- Reuse a policy seed identifier only for intentional cross-mitigation agreement.
